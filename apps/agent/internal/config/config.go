...
func Parse(args []string) (Config, error) {
	...
	gatewayType := fs.String("gateway-type", "clash", "Gateway type: clash, surge or mikrotik")
	...
	if gt != "clash" && gt != "surge" && gt != "mikrotik" {
...
func Usage() string {
	lines := []string{
		"Usage:",
		"  neko-agent --server-url <url> --backend-id <id> --backend-token <token> --gateway-type <clash|surge|mikrotik> --gateway-url <url> [options]",
		"",
		"Required:",
		...
		"  --gateway-type          clash|surge|mikrotik (default clash)",
...
func normalizeGatewayEndpoint(gatewayType, raw string) string {
	...
	if gatewayType == "mikrotik" {
		return strings.TrimSuffix(trimmed, "/rest/ip/firewall/connection")
	}
